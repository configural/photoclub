...
class PhotoController extends Controller {
    ...
    public function showPhoto(Request $request) {
        ...
        if ($photo->ExposureProgram)
            $photo->ExposureProgram = $exposureModes[$photo->ExposureProgram];
        ...
        if ($request->isMethod('post')) {
            if ($request->hasFile('file')) {
                ...
                if ($f = $exif['FocalLength']) {
                    $tmp = explode('/', $f);
                    if ($tmp[1])
                        $exif['FocalLength'] = $tmp[0] / $tmp[1];
                }

                if ($a = $exif['FNumber']) {
                    $tmp = explode('/', $a);
                    if ($tmp[1])
                        $exif['FNumber'] = $tmp[0] / $tmp[1];
                }

                if ($e = $exif['ExposureBiasValue']) {
                    $tmp = explode('/', $e);
                    if ($tmp[1])
                        $exif['ExposureBiasValue'] = round($tmp[0] / $tmp[1], 2);
                }
    ...
    public function rebuildExif() {
        ...
        foreach ($photo as $p) {
            ...
            if (!is_array($exif['ISOSpeedRatings'])) {
                $p->ISOSpeedRatings = $exif['ISOSpeedRatings'];
            } else {
                $p->ISOSpeedRatings = $exif['ISOSpeedRatings'][0];
            }
